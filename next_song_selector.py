...
def choose_next_song(current_song: Song, song_list: list[str], sample_size: int, model_name: str | None = None) -> Song | None:
	...
	prompt += "Critical acclaim 1 limited artistic reputation, 3 respected, 5 highly influential. "
	prompt += "Critical acclaim must reflect artistic influence or reputation, not awards or chart positions."
	...
	prompt += "You may display only a seven number summary per candidate: P,CA,G,I,S,M,T. No explanations. "
	...
	prompt += "\n(7) Respond only with two XML tags and nothing else: "
